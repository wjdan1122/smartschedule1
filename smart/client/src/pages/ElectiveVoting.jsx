...
// تم إبقاء كل الأيقونات لاستخدامها في المكونات الداخلية (مثل التعليقات) وللحفاظ على الدوال
...
function StudentDashboard() {
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (userData && userData.email) { 
                setUser(userData); 
                // ضبط مستوى العرض المبدئي ليتطابق مع الصورة
    ...
    return (
        <div className="dashboard-page">
            {/* تم إزالة fluid="lg" من Container الخارجي لتحقيق تطابق أفضل للخلفية */}
            <Container className="container-custom shadow-lg">
                {/* ==========================================================
                ✅ شريط التنقل (Navbar) مطابق تماماً للصورة
                * إزالة الأيقونات من الروابط
                * تنسيق معلومات المستخدم (الاسم والبريد الإلكتروني على سطرين)
                ========================================================== 
                */}
                <Navbar expand="lg" variant="dark" className="navbar-custom p-3">
                    ...
                    <Navbar.Collapse id="basic-navbar-nav">
                        ...
                        <Nav className="me-auto my-2 my-lg-0 nav-menu">
                            <Nav.Link onClick={() => navigate('/student-dashboard')} className="nav-link-custom active">DASHBOARD</Nav.Link>
                            <Nav.Link onClick={() => navigate('/elective-voting')} className="nav-link-custom">VOTING</Nav.Link>
                        </Nav>
                        {/* معلومات المستخدم وزر الخروج مطابقان للصورة */}
                        <div className="d-flex align-items-center ms-lg-4 mt-3 mt-lg-0">
                            {/* تنسيق معلومات المستخدم ليتطابق مع الصورة (الاسم في الأعلى والبريد في الأسفل) */}
                            <div className="user-info text-white text-end me-3">
                                <div className="user-name fw-bold" style={{ lineHeight: 1.2 }}>{user.name}</div>
                                <div className="user-role" style={{ opacity: 0.8, fontSize: '0.8rem', lineHeight: 1.2 }}>{user.email}</div>
                            </div>
                ...
                <main className="main-content p-4 p-md-5">
                    ...
                    <section className="bg-white rounded-4 p-4 p-md-5 shadow-sm">
                        {/* ==========================================================
                        ✅ محتوى الصفحة مطابق للصورة
                        ========================================================== 
                        */}
                        <h3 className="text-dark mb-3 d-flex align-items-center">
                            {/* تم استخدام أيقونة افتراضية غير مرئية لتوفير مسافة بادئة إذا لزم الأمر، ولكن الأفضل إزالتها للحصول على تطابق تام */}
                            <FaCalendarAlt className="me-2 text-primary" style={{ display: 'none' }} /> 
                            View Schedules
                        </h3>
                        ...
                        <div className="d-flex flex-wrap gap-2">
                            ...
                            {/* زر المستوى التالي - تم إعادته لأنه جزء من منطق البرنامج الأصلي */}
                            <Button
