...
// Added the missing FaUserGraduate icon
...
const ElectiveCourseCard = ({ course }) => {
  ...
  return (
    <Card className="shadow-sm p-3 border-2" style={{ transition: 'all 0.3s ease' }}>
      ...
      <Button onClick={handleManage} variant="success" className="fw-bold">
        Review & Approve 📝
      </Button>
    </Card>
  );
...
const INITIAL_MOCK_DATA = { totalStudents: '...', votingStudents: '...', totalComments: '...', totalVotes: '...', participationRate: '...' };
...
// --- Main Component: Renamed to Dashboard ---
const Dashboard = () => {
  ...
  const fetchData = async (url) => {
    ...
    const response = await fetch(url, { headers: { 'Content-Type': 'application/json', ...(token && { 'Authorization': `Bearer ${token}` }) } });
    if (response.status === 401 || response.status === 403) { throw new Error("Authentication failed. Please log in again."); }
  ...
  const fetchUserInfo = async () => {
    ...
    if (storedUser.full_name && storedUser.role) {
      ...
      setUserInfo({ name: 'Admin User', role: 'Committee Head' });
    }
  };
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      setStats({
        ...
        totalComments: 34, // Mock
        totalVotes: statsData.totalVotes,
        participationRate: participationRate,
      });

      const courses = await fetchData('http://localhost:5000/api/courses/elective');
      const votesPromises = courses.map(course => fetchData(`http://localhost:5000/api/votes/course/${course.course_id}`));
      ...
      const coursesWithVotes = courses.map((course, index) => {
        ...
        return {
          ...
          code: course.dept_code || 'N/A',
          title_ar: course.name,
      ...
      setError(`Failed to load data. Please check server connection and authentication. ${err.message}`);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    return () => { document.body.style.direction = 'ltr'; }; // Keep LTR
  }, [fetchDashboardData]);
  ...
  return (
    <div className="dashboard-page">
      <Alert variant="info" className="text-center m-0 rounded-0">
        <strong>Note:</strong> This dashboard is fetching live data from the server.
      </Alert>
      <Container fluid="lg" className="container-custom shadow-lg">
        <Navbar expand="lg" variant="dark" className="navbar-custom p-3">
          <div className="d-flex align-items-center">
            ...
            <Badge bg="light" text="dark" className="p-2">Admin Dashboard</Badge>
          </div>
          <Navbar.Toggle aria-controls="basic-navbar-nav" />
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className="me-auto my-2 my-lg-0 nav-menu">
              {/* --- CORRECTED NAVIGATION LINKS --- */}
              <Nav.Link onClick={() => navigate('/dashboard')} className="nav-link-custom active rounded-2 p-2 mx-1"><FaHome className="me-2" /> HOME</Nav.Link>
              <Nav.Link onClick={() => navigate('/manageSchedules')} className="nav-link-custom rounded-2 p-2 mx-1"><FaCalendarAlt className="me-2" /> Manage Schedules</Nav.Link>
              ...
              {/* These routes can be added in App.js later */}
              {/* <Nav.Link onClick={() => navigate('/managecourses')} className="nav-link-custom rounded-2 p-2 mx-1"><FaBook className="me-2" /> Course Information</Nav.Link> */}
              {/* <Nav.Link onClick={() => navigate('/notifications')} className="nav-link-custom rounded-2 p-2 mx-1"><FaBell className="me-2" /> Notifications</Nav.Link> */}
            </Nav>
            <div className="d-flex align-items-center ms-lg-4 mt-3 mt-lg-0">
              <div className="text-white text-start me-3">
                <div className="fw-bold">{loading ? 'Loading...' : userInfo.name}</div>
                <div style={{ opacity: 0.8, fontSize: '0.8rem' }}>{userInfo.role}</div>
              </div>
              <Button variant="danger" className="fw-bold" onClick={() => {
                localStorage.clear();
                navigate('/login');
              }}>
        ...
        <main className="main-content p-4 p-md-5">
          <header className="text-center mb-5">
            ...
            <p className="text-secondary fs-6">Manage academic schedules, registration, and planning for the Software Engineering Department.</p>
          ...
          <section className="bg-white rounded-4 p-4 p-md-5 shadow-sm">
            <h3 className="text-dark mb-4 d-flex align-items-center"><FaVoteYea className="me-2 text-primary" /> Elective Course Voting Results</h3>
            <Row xs={1} sm={2} md={3} className="g-3 mb-4">
              <Col><Card className="voting-stat-card-custom h-100 shadow-sm border-0"><Card.Body className="p-3 text-center"><div className="voting-stat-number-custom">{loading ? '...' : stats.totalVotes}</div><div className="text-secondary fw-bold">Total Votes</div></Card.Body></Card></Col>
              <Col><Card className="voting-stat-card-custom h-100 shadow-sm border-0"><Card.Body className="p-3 text-center"><div className="voting-stat-number-custom">{loading ? '...' : stats.participationRate + '%'}</div><div className="text-secondary fw-bold">Participation</div></Card.Body></Card></Col>
              <Col><Card className="voting-stat-card-custom h-100 shadow-sm border-0"><Card.Body className="p-3 text-center"><div className="voting-stat-number-custom">5</div><div className="text-secondary fw-bold">Days Left (Mock)</div></Card.Body></Card></Col>
            </Row>
            {loading && <div className="text-center my-4"><Spinner /><p className="mt-2">Loading voting data...</p></div>}
            {!loading && votingCourses.length === 0 && (<Alert variant="warning" className="text-center">No elective courses or voting data found.</Alert>)}
            <Row xs={1} md={3} className="g-4">
              {votingCourses.map(course => (<Col key={course.course_id}><ElectiveCourseCard course={course} /></Col>))}
            </Row>
          </section>

          <section className="bg-white rounded-4 p-4 p-md-5 shadow-sm mt-5">
            <h3 className="text-dark mb-4 d-flex align-items-center"><FaBell className="me-2 text-primary" /> Recent Notifications (Mock)</h3>
            <div>
              {DUMMY_NOTIFICATIONS.map((notification, index) => (<NotificationItem key={index} notification={notification} />))}
            </div>
          </section>
        </main>
      </Container>
